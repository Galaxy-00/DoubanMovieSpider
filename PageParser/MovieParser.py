...
class MovieParser(object):
    ...
    def __get_script_writers(self):
        try:
            res = re.search(r"<span.*<span class='pl'>编剧</span>[\s\S]*?</span>", self.__html_doc).group(0)
    ...
    def __get_actors(self):
        try:
            self.__movie_info['actors'] = self.__pq_doc.find('.actor .attrs').text()
        except:
            pass
    ...
    def __get_release_region(self):
        try:
            res = re.search(r'<span class="pl">制片国家/地区:</span> (.*)<br/>', self.__html_doc).group(1)
    ...
    def __get_languages(self):
        try:
            res = re.search(r'<span class="pl">语言:</span> (.*)<br/>', self.__html_doc).group(1)
    ...
    def __get_short_comment_num(self):
        try:
            res = self.__pq_doc.find('.mod-hd .pl').text()
            self.__movie_info['shortCommentNum'] = re.search(r'([0-9]+)', res).group(0)
        except:
            pass

    def __get_show_comment_num(self):
        try:
            res = self.__pq_doc.find('#reviews-wrapper .pl').text()
            self.__movie_info['showCommentNum'] = re.search(r'([0-9]+)', res).group(0)
        except:
            pass
